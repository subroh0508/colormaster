...
**EPIC-A3 完走** (A3-0 + A3-1〜A3-14 全 15 PR マージ済、2026-05-18)。Phase A の専用 Skill 群 13 種が active、汎用 `harness-bootstrap` archived 化済。`roadmap-tracker` Skill が `docs/harness/plan.md` §6 + Epic ロードマップから自動更新する想定 (Phase 8 自動起動フックは A3-12 で本格化済、本 mirror PR で初回ドッグフード)。次の推奨着手:
...
2. **A5 不要モジュール撤去 / A8 im@sparql Docker 環境構築** — A3 と独立に着手可能、touch ファイル重複ゼロで A4 と並走可。A5 は `js/**` / `kotlin-js-store/**` / `public/**` / `core/network/{auth,firestore}/**` / `firebase.json` 等の旧 Web app 残骸削除、A8 は `docker-compose.yml` + `docs/runbooks/local-imasparql.md` + `backend/**` の Fuseki ローカル環境
3. **A6 Lint / Format 基盤** — Spotless + ktlint + detekt + Konsist + markdownlint-cli2 + Gradle カスタムタスク (frontmatter 検証 / 5 行 summary 検証 / 日本語見出し検証 / 配列 block 形式検証) + trufflehog の本格導入、A2-3 で `docs-structure.md` / `roadmap.md` / `pii.md` / `secrets.md` 等が想定する機械検証を実体化。A3 完了で各 rule の SoT が安定したため着手可
